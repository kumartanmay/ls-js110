...
{
  //Code goes here!
  //at each iteration, slice the array into at the index position
  //compare the sum of both, if same, return the index
  //else, continue
  //return -1, if there is no index returned in iteration
  
  for(let index = 0; index < arr.length; index++){
    let arrLeft = []
    if(index === 0) arrLeft = [];
    else arrLeft = arr.slice(0, index);
    let arrRight = arr.slice(index+1,)
    
    if(sum(arrLeft) === sum(arrRight)) return index;
  }
  return -1;
}
